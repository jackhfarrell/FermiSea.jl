# 1: Installation
To the extent there is anyone interested in this code at all, I expect they will be relatively new to Julia.  This first tutorial just gives my reccomendations to how to get going with Julia and download this package. 
...
```bash
git clone https://github.com/jackhfarrell/FermiSea.jl
cd FermiSea.jl
```
...
This downloads the Julia packages listed in `Project.toml` and `Manifest.toml`.  To make startup a little faster when you actually try and run a script, you can also run
...
time stepping, mesh generation, and Literate.jl output. Add those to this same
repo-root environment as needed:
